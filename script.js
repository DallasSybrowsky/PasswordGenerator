...
function generatePassword() {
  ...
  while (validEntry === false) {
    var passwordLength = prompt(
      "How many characters would you like in your password? Please enter an number between 8 and 128."
    );
  ...
  var useUpper = confirm(
    "Would you like to use uppercase characters? Click OK or Cancel."
  );
  ...
  // Conditional statement governing various responses
  if (useNumber && useUpper) {
    pwChoices = alphaLower.concat(alphaUpper,numbers);
  } else if(useNumber && !useUpper) {
    pwChoices = alphaLower.concat(numbers)
  } else if (!useNumber && useUpper) {
    pwChoices = alphaLower.concat(alphaUpper);
  } else if (!useNumber && !useUpper) {
    pwChoices = alphaLower;
  ...
  if (useUpper && useNumber && useSpecial) { 
    ...
    pwChoices = alphaLower.concat(alphaUpper,specials);
  } else if (!useUpper && useNumber && useSpecial) {
    pwChoices = alphaLower.concat(numbers,specials);
  } else if (!useUpper && !useNumber && useSpecial) {
    pwChoices = alphaLower.concat(specials);
  } else if (!useUpper && useNumber && !useSpecial) {
    pwChoices = alphaLower.concat(numbers); 
  } else {
    pwChoices = alphaLower;
